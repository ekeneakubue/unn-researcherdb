...
import type { AdminResearchDetail, ResearchPersonInput, ResearchStatusLabel } from "@/lib/research-shared";
import { researchStatusOptions } from "@/lib/research-shared";

type ResearchDetailModalProps = {
  open: boolean;
  loading: boolean;
  detail: AdminResearchDetail | null;
  statusSaving?: boolean;
  onClose: () => void;
  onStatusChange: (status: ResearchStatusLabel) => void;
};
...
export function ResearchDetailModal({
  ...
  function handleStatusChange(event: React.ChangeEvent<HTMLSelectElement>) {
    onStatusChange(event.target.value as ResearchStatusLabel);
  }
          ...
          <div>
            ...
            {detail ? (
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <StatusBadge
                  label={detail.status}
                  styles={researchStatusStyles}
                />
                <label htmlFor={statusId} className="flex items-center gap-2 text-sm text-unn-muted">
                  <span>Status</span>
                  <select
                    id={statusId}
                    value={detail.status}
                    ...
                    className="h-9 rounded-full border border-unn-green/15 bg-white px-3 text-sm font-medium text-unn-ink outline-none focus:border-unn-gold disabled:opacity-60"
                  >
...
function formatPeopleList(people: ResearchPersonInput[]) {
