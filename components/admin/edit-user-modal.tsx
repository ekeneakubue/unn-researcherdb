...
function toForm(user: AdminUser, roles: string[]) {
  const role =
    roles.includes(user.role) || user.role === "Super-admin"
      ? user.role
      : (roles[0] ?? user.role);
...
export function EditUserModal({
  ...
  useEffect(() => {
    ...
    if (open && user) {
      setForm(toForm(user, assignableRoles));
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!user) return;
    if (!roleOptions.includes(form.role)) return;
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-unn-muted">
              Update this portal account. Leave password blank to keep the current one.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block text-sm sm:col-span-2">
            ...
            <input
              ...
              className="mt-1.5 h-11 w-full rounded-xl border border-unn-green/15 px-3 text-sm outline-none focus:border-unn-gold"
              placeholder="Office of Research"
            />
          </label>
          <div className="sm:col-span-2">
            <label htmlFor={passwordId} className="block text-sm">
              New password <span className="text-unn-muted">(optional)</span>
            </label>
