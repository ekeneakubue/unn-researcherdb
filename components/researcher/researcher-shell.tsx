...
import { ResearcherLogoutButton } from "@/components/researcher/researcher-logout-button";
import { UnnCrest } from "@/components/unn-crest";
import {
  getResearcherNav,
  getResearcherPageTitle,
  type ResearcherProfileContext,
} from "@/lib/researcher-portal-config";

export type { ResearcherProfileContext };

const ResearcherPortalContext = createContext<ResearcherProfileContext>({
  name: "Researcher",
  email: "",
  faculty: "",
  reference: null,
...
export function ResearcherPortalProvider({
  ...
  return (
    <ResearcherPortalContext.Provider value={profile}>
      {children}
    </ResearcherPortalContext.Provider>
  );
}

export function useResearcherPortal() {
  return useContext(ResearcherPortalContext);
}
...
export function ResearcherMenuProvider({ children }: { children: React.ReactNode }) {
...
export function ResearcherSidebar() {
  ...
  const profile = useResearcherPortal();
  const nav = getResearcherNav();
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex flex-1 flex-col gap-1 p-3" aria-label="Researcher dashboard">
          {nav.map((item) => {
            const active =
              item.segment === ""
                ? pathname === item.href
        ...
        <div className="border-t border-white/10 p-4 text-xs text-white/70">
          ...
          <p className="mt-1">{profile.faculty}</p>
          {profile.reference ? (
            <p className="mt-1 font-mono text-[11px] text-unn-gold-soft">{profile.reference}</p>
          ) : null}
          <Link
            href="/"
            className="mt-3 block rounded-xl px-3 py-2 text-sm text-unn-gold-soft hover:bg-white/8"
          >
            View public site
          </Link>
          <ResearcherLogoutButton />
        </div>
      </aside>
    </>
  );
...
export function ResearcherTopbar() {
  const pathname = usePathname();
  const profile = useResearcherPortal();
  const title = getResearcherPageTitle(pathname);
  const { open, setOpen } = useContext(MenuContext);
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between gap-4 border-b border-unn-green/10 bg-unn-cream/90 px-4 py-3 backdrop-blur-md sm:px-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-[11px] uppercase tracking-[0.18em] text-unn-gold">My workspace</p>
